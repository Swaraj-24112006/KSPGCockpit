...
export default function KaizenSheetForm({
  ...
  const [minifactory, setMinifactory] = useState(initialData?.minifactory || '');
  ...
  const [ideaBy, setIdeaBy] = useState(initialData?.ideaBy || '');
  const [implementedBy, setImplementedBy] = useState(initialData?.implementedBy || '');
  ...
  // Photos — null means no photo uploaded yet (no placeholder/sample)
  const [photoBefore, setPhotoBefore] = useState<string | null>(
    initialData?.photoBeforeUrl || (initialData as any)?.photo_before_url || initialData?.photoBefore || null
  );
  const [photoAfter, setPhotoAfter] = useState<string | null>(
    initialData?.photoAfterUrl || (initialData as any)?.photo_after_url || initialData?.photoAfter || null
  );
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setMinifactory(initialData.minifactory || '');
      ...
      // Photos from saved draft
      const beforeImg = initialData.photoBeforeUrl || (initialData as any)?.photo_before_url || initialData.photoBefore;
      if (beforeImg) {
        setPhotoBefore(beforeImg);
      } else {
        setPhotoBefore(null);
        setPhotoBeforeFile(null);
      }
      if (initialData.photoAfterUrl || (initialData as any)?.photo_after_url || initialData.photoAfter) {
        setPhotoAfter(initialData.photoAfterUrl || (initialData as any)?.photo_after_url || initialData.photoAfter);
      ...
      setDraftSrNo(undefined);
      setTitle('');
      setProblemBefore('');
      setCounterMeasureAfter('');
      setResult('');
      setArea('');
      setMinifactory('');
      setLocation('');
      setMachine('');
      setSuggestionDate(new Date().toISOString().split('T')[0]);
      setClosingTargetDate('');
      setImplementedDate('');
      setIdeaBy('');
      setImplementedBy('');
      setPreparedBy('');
      setBenefits({ p: false, q: false, c: false, d: false, s: false, m: false });
      setCostSave(0);
      setPhotoBefore(null);
      setPhotoAfter(null);
      setPhotoBeforeFile(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isBeforePhotoMissing = !photoBefore;
    const isAfterPhotoMissing = !photoAfter;
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-slate-50 border border-slate-150 rounded-xl">
          <div>
            ...
            >
              <option value="">Select Mini-Factory...</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Originator Name"
              />
            </div>
            <div>
